...
<body>
    ...
    <form method="GET">
        <select name="name">
            ...
            <?php foreach ($clanky as $clanek): ?>
                <option value="<?= htmlspecialchars($clanek["name"]) ?>"
                    <?= isset($_GET["name"]) && $_GET["name"] === $clanek["name"] ? 'selected' : '' ?>>
                    <?= htmlspecialchars($clanek["name"]) ?>
                </option>
            <?php endforeach; ?>
        </select>
    ...
    if (isset($_GET["name"]) && $_GET["name"] !== "") {
        ...
        foreach ($clanky as $test) {
            if ($test["name"] === $search || $test["surname"] === $search) {
                $filter[] = $test;
            }
        }
    } else {
    ...
    <table id="customers">
        ...
        <?php foreach ($filter as $clanek): ?>
            <tr>
                ...
                <td><?= htmlspecialchars($clanek["surname"]) ?></td>
            </tr>
        <?php endforeach; ?>
    </table>
</body>
